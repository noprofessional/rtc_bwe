...
public:
    ...
    //                      Initial window size is set to this, but may be
    //                      changed to something lower by calling
    //                      SetWindowSize().
    ...
    // Note that despite this being a const method, it still updates the
    // internal state (moves averaging window), but it doesn't make any
    // alterations that are observable from the other methods, as long as
    ...
    optional<int64_t> Rate(int64_t now_ms) const;
