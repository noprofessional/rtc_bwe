...
optional<int64_t> RateStatistics::Rate(int64_t now_ms) const
{
    // Yeah, this const_cast ain't pretty, but the alternative is to declare
    // most of the members as mutable...
    ...
    if (first_timestamp_ != -1)
    {
        ...
        {
            // Count window as full even if no data points currently in view, if
            // the data stream started before the window.
            active_window_size = current_window_size_ms_;
        }
        else
        {
            // Size of a single bucket is 1ms, so even if now_ms ==
            // first_timestmap_ the window size should be 1.
...
void RateStatistics::EraseOld(int64_t now_ms)
{
    ...
    while (!buckets_.empty() && buckets_.front().timestamp < new_oldest_time)
    {
        ...
        // TODO(https://bugs.webrtc.org/11247): Consider if overflow_ can be
        // reset.
    }
}
...
bool RateStatistics::SetWindowSize(int64_t window_size_ms, int64_t now_ms)
{
    ...
    if (first_timestamp_ != -1)
    {
        ...
        first_timestamp_ = std::max(first_timestamp_,
                                    now_ms - window_size_ms + 1);
    }
